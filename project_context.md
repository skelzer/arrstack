...
* **Wake targets:** `TARGETS[]` in `esp32/esp32_wol/config.h` — one `{ id, mac, ip }` per machine (currently just `server`). Each `id` must match a button target in `cloudflare-worker/src/index.js`.
