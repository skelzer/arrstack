...
6. `space-check.ps1` runs every 6 hours (task "ArrStack Space Check"): when F: drops under 150 GB it switches the Seerr default root folders to the D: overflow folders (`/data/Movies`, `/data/Shows`) and sends a Telegram message via `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` from `.env`. It also alerts when a container is down or cloudflared reports no tunnel connections (its readiness endpoint on `127.0.0.1:20241/ready`). Radarr/Sonarr also refuse imports below 50 GB free.
10. `telegram-bot.ps1` (task "ArrStack Telegram Bot", at logon) long-polls the same bot for commands from allowed chat IDs (`TELEGRAM_CHAT_ID` plus optional `TELEGRAM_ALLOWED_IDS`): `/status`, `/np`, `/queue`, `/movienight [genre]` (random unwatched movie with poster), `/tdarr pause|resume`, `/digest`, `/sleep` (refused while something is playing). Only one process may poll a Telegram bot, so do not run a second copy.
